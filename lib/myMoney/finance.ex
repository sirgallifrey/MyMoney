defmodule MyMoney.Finance do
  ...
  def list_transactions() do
    Repo.all(from t in Transaction,
      ...
      select_merge: %{account_name: a.name}
    )
  end
  ...
  def list_accounts_options(user_id) do
    Repo.all(from a in Account,
      where: a.user_id == ^user_id,
      select: [:id, :name]
    )
    |> Account.to_form_options
  end
  ...
  def create_account(attrs \\ %{}) do
    create_account_changeset(attrs)
    |> Repo.insert()
  end

  @doc """
  Creates a account changeset, without inserting.
